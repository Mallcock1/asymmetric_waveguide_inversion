...
class fibril:
    def __init__(self, yb, yt):
        ...
        self.t_vals_2 = np.arange(len(self.yb)) * cad
        self.t_vals_cont_2 = np.linspace(0, self.t_vals_2[-1], 1000)
    ...
    #fit a sinusoidal curve
    def sin_fitting_2(self, N=3, trend_type="poly"):
        def sin_func_2(x, a, b, c): #a = amplitude, b = angular frequency
            return a * np.sin(b * x + c)
        yb_params, yb_params_covariance = curve_fit(sin_func_2, self.t_vals_2, self.detrend(N, trend_type)[0], p0=[300, 0.03, 0])
        yt_params, yt_params_covariance = curve_fit(sin_func_2, self.t_vals_2, self.detrend(N, trend_type)[1], p0=[300, 0.03, 0])
        return [sin_func_2(self.t_vals_cont_2, yb_params[0], yb_params[1], yb_params[2]), yb_params,
                sin_func_2(self.t_vals_cont_2, yt_params[0], yt_params[1], yt_params[2]), yt_params]
        
    #fit a sinusoidal curve
    def sin_fitting_3(self, N=3, trend_type="poly"):
        def sin_func_3(x, a, b, c): #a = amplitude, b = angular frequency
            return a * np.sin(b * x + c)
        yb_params, yb_params_covariance = curve_fit(sin_func_3, self.t_vals_2, self.detrend(N, trend_type)[0], p0=[100, 0.03, 0])
        yt_params, yt_params_covariance = curve_fit(sin_func_3, self.t_vals_2, self.detrend(N, trend_type)[1], p0=[100, 0.03, 0])
        return [sin_func_3(self.t_vals_cont_2, yb_params[0], yb_params[1], yb_params[2]), yb_params,
                sin_func_3(self.t_vals_cont_2, yt_params[0], yt_params[1], yt_params[2]), yt_params]
